 ...
 */
function toggleElement(element, displayFormat) {
 ...
 */
function slideElement(element, mSecs, direction) {
 ...
 */
function decapitalizeString(string) {
 ...
 */
function getSubTypeProperties(filters) {
    return new Promise((resolve, reject) => {
        // Create query string from searchConditions
        let filterString = '';
        if (filters) {
 ...
 */
function createPropertyInput(typeProperty, inputLocation) {
    let returnElement;
    switch (typeProperty.POSSIBLEVALUES) {
        case 'string':
            ...
            returnElement.autocomplete = 'off';
            break;
        case 'number':
            returnElement = document.createElement('input');
 ...
 * @returns {HTMLElement}
 */
function createTableElementFromArray(dataArray) {
