 ...
 * @memberof report
 */
    export let userInfo: { id: string; roleId: string; divisionId: string; admin: string };
}
/**
 * @member graphObject
 ...
 * @memberof report
 */
let graphObject: { [key: string]: any } = {};
...
namespace Report {
    ...
    (document.getElementById('sub_title') as HTMLElement).innerText = changeTo.split('_')[0];
 ...
 */
function adjustSubMenu(
    searchReportsButton: boolean,
    searchNodesButton: boolean,
    addTextButton: boolean,
    containerButton: boolean,
    saveReportButton: boolean,
    editReportButton: boolean,
    exportReportButton: boolean,
    deleteReportButton: boolean
): void {
    // Display menu elements, which are needed
    (document.getElementById('search_reports_button') as HTMLElement).style.display = searchReportsButton ? 'flex' : 'none';
    (document.getElementById('report_id_display') as HTMLElement).style.display = searchReportsButton ? 'flex' : 'none';
    (document.getElementById('search_nodes_button') as HTMLElement).style.display = searchNodesButton ? 'flex' : 'none';
    (document.getElementById('add_report_element_button') as HTMLElement).style.display = addTextButton ? 'flex' : 'none';
    (document.getElementById('select_container_button') as HTMLElement).style.display = containerButton ? 'flex' : 'none';
    (document.getElementById('container_id_display_div') as HTMLElement).style.display = containerButton ? 'flex' : 'none';
    (document.getElementById('save_report_button') as HTMLElement).style.display = saveReportButton ? 'flex' : 'none';
    (document.getElementById('edit_report_button') as HTMLElement).style.display = editReportButton ? 'flex' : 'none';
    (document.getElementById('export_report_button') as HTMLElement).style.display = exportReportButton ? 'flex' : 'none';
    (document.getElementById('delete_report_button') as HTMLElement).style.display = deleteReportButton ? 'flex' : 'none';
}
 ...
 */
function createActiveNodeListItem(nodeList: Array<{ dataId: string; type: string }>): void {
    const nodeListContainer = document.getElementById('node_list') as HTMLElement;
 ...
 */
function createDisplayNodeListItems(nodeList: Array<{ dataId: string; type: string; content: string }>, elementsAreActive: boolean): void {
    const nodeListContainer = document.getElementById('node_list') as HTMLElement;
 ...
 */
function addReportElement(node: any): void {
    const reportContentDiv = document.getElementById('report_content') as HTMLElement;
    ...
    fetch(`/data/getDataContent?dataId=${nodeId}`)
        ...
        .then(data => {
            ...
            if (nodeType === 'text') {
                ...
                const chartElement = document.getElementById(`chart_type_${reportElementId}`) as HTMLSelectElement | null;
                if (chartElement) {
 ...
 * @param {boolean} elementsAreActive True for edit_report tab; If true, activates the node list item
 */
function displayReportElement(node: any, elementsAreActive: boolean): void {
    const reportContentDiv = document.getElementById('report_content') as HTMLElement;
            ...
            if (node.content.xAxis) {
                ...
                if (xAxisElement) {
                    ...
                    for (let i = 0; i < xOptions.length; i++) {
                        if (node.content.xAxis === (xOptions[i] as HTMLOptionElement).innerText) {
                            (xOptions[i] as HTMLOptionElement).selected = true;
                            break;
                        }
                    }
                }
            ...
            if (node.content.yAxis) {
                ...
                if (yAxisElement) {
                    ...
                    for (let i = 0; i < yOptions.length; i++) {
                        if (node.content.yAxis === (yOptions[i] as HTMLOptionElement).innerText) {
                            (yOptions[i] as HTMLOptionElement).selected = true;
                            break;
                        }
                    }
                }
 ...
 */
function populateXYOptions(reportElementId: string): void {
    const data = graphObject[reportElementId]
    const axisOptions = Object.keys(data.data[0])

    const xAxis = document.getElementById(`x_axis_${reportElementId}`) as HTMLSelectElement;
    const yAxis = document.getElementById(`y_axis_${reportElementId}`) as HTMLSelectElement;
    xAxis.innerHTML = ''
    yAxis.innerHTML = ''
 ...
 * @param {string} reportElementId Id of the graph element
 */
function changeChart(element: HTMLSelectElement, reportElementId: string): void {
 ...
 * @param {string} reportElementId Id of the graph element
 */
function changeAxis(element: HTMLSelectElement, reportElementId: string): void {
 ...
 * @param {string} reportElementId Id of the graph element
 */
function generateGraph(reportElementId: string): void {
    let chartType = graphObject[reportElementId].chart
    let xData = graphObject[reportElementId].xData
    let yData = graphObject[reportElementId].yData

    let graphElement = document.getElementById(`graph_${reportElementId}`) as HTMLElement;
 ...
 */
function removeReportElementAndNodeItem(nodeId: string): void {
    const dataType = document.getElementById(`node_item_${nodeId}`)?.dataset.type;

    // Remove report element
    document.getElementById(`report_element_${nodeId}`)?.remove()

    // Remove node item from list
    document.getElementById(`node_item_${nodeId}`)?.remove()
 ...
 * @memberof report
 */
function removeContainerDisplay(): void {
 ...
 */
function addReportElementCleanup(nodeId: string): void {
 ...
 */
function makeElementDraggableAndResizable(newElement: HTMLElement): void {
 ...
 */
function doesOverlap(element: { id: string; top: number; left: number; bottom: number; right: number }): boolean {
    ...
    for (let i = 0; i < reportElements.length; i++) {
        ...
        const comparisonElementTop = (reportElements[i] as HTMLElement).offsetTop
        const comparisonElementLeft = (reportElements[i] as HTMLElement).offsetLeft
        const comparisonElementBottom = (reportElements[i] as HTMLElement).offsetTop + (reportElements[i] as HTMLElement).offsetHeight
        const comparisonElementRight = (reportElements[i] as HTMLElement).offsetLeft + (reportElements[i] as HTMLElement).offsetWidth

        // Return true if elements would overlap
        if (
            (
                (element.left <= comparisonElementLeft && element.right >= comparisonElementRight) ||       // if left and right are surrounding the comparisonElement
 ...
 */
function staysInsideContainer(element: { top: number; left: number; bottom: number; right: number; height: number; width: number }): boolean {
 ...
 * @memberof report
 */
async function openReportSearchPopup(): Promise<void> {
    ...
    await populateSubTypeProperties('report', 'report_search')
 ...
 * @memberof report
 */
async function openNodeSearchPopup(): Promise<void> {
    await populateNodeSearchSubTypeSelect()
 ...
 * @memberof report
 */
async function openSelectContainerPopup(): Promise<void> {
    await populateContainerSelectSubTypeSelect()
 ...
 * @memberof report
 */
async function populateNodeSearchSubTypeSelect(): Promise<void> {
    ...
    getTypeNames(filters)
        .then(typeNames => {
            ...
            const filteredTypes = typeNames.filter((type: { TYPENAME: string; }) => allowedTypes.includes(type.TYPENAME))
            ...
            if (subTypeSelect) {
                populateSubTypeProperties((subTypeSelect as HTMLSelectElement).value, 'node_search');
            }
        })
        .catch((error) => {
            console.error(error)
 ...
 * @memberof report
 */
async function populateContainerSelectSubTypeSelect(): Promise<void> {
    // Get all data sub types
    const filters = {
        nodeType: {
            equal: true,
    ...
    getTypeNames(filters)
        .then(typeNames => {
            ...
            const filteredTypes = typeNames.filter((type: { TYPENAME: string; }) => type.TYPENAME !== 'report')
            ...
            // populateSubTypeProperties with the properties of the first of those types
            if (subTypeSelect) {
                populateSubTypeProperties((subTypeSelect as HTMLSelectElement).value, 'container_select');
            }
        })
        .catch((error) => {
            console.error(error)
 ...
 */
async function populateSubTypeProperties(subType: string, popup: string): Promise<void> {
    const filters = {
        typeName: subType
    }
    ...
    getSubTypeProperties(filters)
        .then(result => {
            ...
            const propertyContainer = document.getElementById(`${popup}_popup_property_container`) as HTMLElement;
 ...
 */
async function populateSearchPopupList(popup: string): Promise<void> {
    let subType: string;
    if (popup === 'node_search') {
        subType = (document.getElementById('node_search_popup_sub_type_select') as HTMLSelectElement).value;
    } else if (popup === 'container_select') {
        subType = (document.getElementById('container_select_popup_sub_type_select') as HTMLSelectElement).value;
    } else {
        subType = 'report'
    }
    ...
    for (const property of subTypeProperties) {
        if (property.POSSIBLEVALUES === 'boolean') {
            const trueRadio = document.getElementById(`${popup}_popup_${property.PROPERTYNAME}_input_true`) as HTMLInputElement
            const falseRadio = document.getElementById(`${popup}_popup_${property.PROPERTYNAME}_input_false`) as HTMLInputElement
            ...
            const propertyInput = document.getElementById(`${popup}_popup_${property.PROPERTYNAME}_input`) as HTMLInputElement
    ...
    getNodes(filters)
        .then(result => {
            ...
            if (popup === 'node_search') {
                ...
                const listedNodesIds: string[] = [];
                for (const listedNode of Array.from(listedNodes)) {
                    listedNodesIds.push((listedNode as HTMLElement).innerText)
                }
                if (listedNodesIds.length > 0) {
                    nodesList = nodesList.filter((node: { ID: string; }) => !listedNodesIds.includes(node.ID))
                }
            } else if (popup === 'container_select') {
                ...
                if (displayedContainerId) {
                    nodesList = nodesList.filter((node: { ID: string; }) => node.ID !== displayedContainerId)
                }
            } else {
                ...
                if (displayedReportId) {
                    nodesList = nodesList.filter((node: { ID: string; }) => node.ID !== displayedReportId)
                }
            }

            const popupNodesList = document.getElementById(`${popup}_popup_nodes_list`) as HTMLElement;
            ...
            for (const node of nodesList) {
                ...
                popupNodeElement.onclick = function () {
                    if (!(this as HTMLElement).classList.contains('popup_selected_element') && (popup === 'report_search' || popup === 'container_select')) {
                        // Check if selected_element is contained by a sibling, if yes, unselect it
                        const selectedSibling = getSelectedSibling(this as HTMLElement, popup)
                        if (selectedSibling) {
 ...
 * @memberof report
 */
async function populateReportSearchPopupList(): Promise<void> {
    const properties: { [key: string]: any } = {
        type: {
            equal: true,
            value: 'report'
    ...
    for (const property of subTypeProperties) {
        if (property.POSSIBLEVALUES === 'boolean') {
            ...
            const propertyInput = document.getElementById(`${popup}_popup_${property.PROPERTYNAME}_input`) as HTMLInputElement | HTMLSelectElement;
            if (propertyInput.tagName === 'INPUT' && propertyInput.value) {
                properties[property.PROPERTYNAME] = {
                    equal: true,
                    value: propertyInput.value
    ...
    const createdByInput = (document.getElementById('report_created_by') as HTMLInputElement).value;
    if (createdByInput !== '') {
    ...
    const containedByInput = (document.getElementById('report_contained_by') as HTMLInputElement).value;
    if (containedByInput !== '') {
        filters.containedBy = {
            equal: true,
    ...
    fetch('/report/getReportNodes', {
        ...
        .then(result => {
            ...
            const popupNodesList = document.getElementById('report_search_popup_nodes_list') as HTMLElement;
            popupNodesList.innerHTML = ''
            ...
            for (const node of nodesList) {
                ...
                popupNodeElement.onclick = function () {
                    if (!(this as HTMLElement).classList.contains('popup_selected_element')) {
                        // Check if selected_element is contained by a sibling, if yes, unselect it
                        const selectedSibling = getSelectedSibling(this as HTMLElement, 'report_search')
                        if (selectedSibling) {
                            if (selectedSibling instanceof HTMLElement) {
                                selectedSibling.classList.remove('popup_selected_element');
                            }
 ...
 */
function getSelectedSibling(element: HTMLElement, popup: string): HTMLElement | boolean {
 ...
 */
function selectFromReportSearchPopup(comingFromCreate?: string): void {
    ...
    fetch(`/report/get?reportId=${selectedReportId}`)
        ...
        .then(json => {
            ...
            const reportIdDisplay = document.getElementById('report_id_display') as HTMLElement;
            reportIdDisplay.innerText = selectedReportId

            if (json.body.container) {
                // Add container id to the container id display
                const containerIdDisplay = document.getElementById('container_id_display') as HTMLElement;
 ...
 * @memberof report
 */
function selectFromNodeSearchPopup(): void {
    const selectedElements: { dataId: string; type: string }[] = [];
    ...
    const dataType = (document.getElementById('node_search_popup_sub_type_select') as HTMLSelectElement).value;
    for (const element of Array.from(selectedPopupElements)) {
        selectedElements.push({
            dataId: (element as HTMLElement).innerText,
            type: dataType
        })
    }
 ...
 * @memberof report
 */
function selectFromContainerSelectPopup(): void {
    const selectedPopupElements = document.getElementsByClassName('popup_selected_element')
    ...
    document.getElementById('container_id_display')!.innerText = (selectedPopupElements[0] as HTMLElement).innerText;
 ...
 */
function closePopup(popup: string): void {
    ...
    if (popup === 'report_search') {
        (document.getElementById('report_created_by') as HTMLInputElement).value = '';
        (document.getElementById('report_contained_by') as HTMLInputElement).value = '';
    }
}
 ...
 */
function getReportLayoutAndContainedDataIds(reportElements: HTMLCollectionOf<Element>): { reportLayout: any[], containedDataIds: string[] } {
    const reportLayout: any[] = [];
    const containedDataIds: string[] = [];
    ...
    for (const element of Array.from(reportElements)) {
        ...
        if (element.querySelector('.graph_container')) {
            ...
            saveObject.content = (element.querySelector('.report_text') as HTMLElement).innerText;
            saveObject.dataId = dataId
            containedDataIds.push(dataId)
        } else if (element.querySelector('.report_text_custom')) {
            saveObject.type = 'input'
            saveObject.content = (element.querySelector('.report_text_custom') as HTMLTextAreaElement).value;
 ...
 */
function getDifferentDataIds(oldLayout: any[], newLayout: any[]): { addedDataIds: string[], removedDataIds: string[] } | boolean {
    const addedDataIds: string[] = [];
    const removedDataIds: string[] = [];

    // Get all the dataIds of each layout
    const oldDataIds: string[] = [];
 ...
 */
function deleteReport(alreadyConfirmed: boolean): void {
 ...
 */
function checkForSave(movePage: string | boolean, changeTab: string | boolean): boolean {
    ...
    if (movePage) {
        location.href = movePage as string;
    } else if (changeTab) {
        Report.changeToTab(changeTab as string, true);
    } else {
        return true;
    }
    return true;
 ...
 * @memberof report
 */
function resetPopup(popup: string): void {
    ...
    for (const input of Array.from(propertyInputs) as HTMLInputElement[]) {
    ...
    for (const select of Array.from(propertySelects) as HTMLSelectElement[]) {
    ...
    if (popup === 'node_search') {
        const subTypeSelect = document.getElementById('node_search_popup_sub_type_select') as HTMLSelectElement;
    ...
    if (popup === 'container_select') {
        const subTypeSelect = document.getElementById('container_select_popup_sub_type_select') as HTMLSelectElement;
        subTypeSelect.value = subTypeSelect.options[0].value
 ...
 */
function getNextCustomTextId(): number {
    ...
    const customTextNodes = Array.from(nodeListItems).filter(item => {
        ...
        return titleElement ? /Custom Text/.test((titleElement as HTMLElement).innerText) : false;
    })
    ...
    return parseInt((customTextNodes[customTextNodes.length - 1] as HTMLElement).innerText.split(' ')[2]) + 1
}
 ...
 */
function addContentToSlide(presentation: any, slide: any, reportLayout: any[]): void {    for(const element of reportLayout) {
        if (element.type === 'input' || element.type === 'text') {
            ...
            if (imageElement) {
                const imageData = getImageData(imageElement as HTMLImageElement);
 ...
 */
function applyRatio(originalValue: number, ratioType: string): number {
 ...
 */
function getImageData(imageElement: HTMLImageElement): string {
