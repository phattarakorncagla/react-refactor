...
const ICON_HEIGHT_DIFF = 75,
  ICON_TOP_OFFSET = 0.8,
  TIMELINE_WIDTH = 80,
  NODE_DISTANCE = 0.73,
  NODE_DISTANCE_OFFSET = 0.12,
  RADIUS = 17;

const width = (document.getElementById("canvas_container") as HTMLElement)
    .offsetWidth,
  height = (document.getElementById("canvas_container") as HTMLElement)
    .offsetHeight,
  iconEachDayDiff = width / 80,
  monthArr = [
    "Jan",
    "Feb",
    "Mar",
 ...
 */
function setGraph(xAxisName: string, yAxisName: string): void {
  const xDropdownStyle = (document.getElementById("dropdown_x") as HTMLElement)
    .style;
  const yDropdownStyle = (document.getElementById("dropdown_y") as HTMLElement)
    .style;
  const xLabelContainerStyle = (
    document.querySelector(".x_label_container") as HTMLElement
  ).style;
  const yLabelContainerStyle = (
    document.querySelector(".y_label_container") as HTMLElement
  ).style;
  ...
  const urlQueryString = createUrlQueryStringFromSearchConditions(
    xAxisName,
    yAxisName
  );
  ...
  d3.json(`/graph/getGraphNodes${urlQueryString}`, {
    ...
    graph = res.body;
    const findDataset = graph.nodes.find((el: any) => el.label == "DATASET");

    if (
      findDataset == undefined &&
      graph.graph != "BYID" &&
      graph.graph != "data"
    ) {
      (document.getElementById("time_line") as HTMLElement).style.opacity = "0";
 ...
 * @param {string} yAxisName
 */
function createGraph(xAxisName: string, yAxisName: string): void {
  ...
  for (let i = 0; i < yObjArr.length; i++) {
    ...
    if (i != 0) {
      if (yObjArr[i].yAxis != yObjArr[i - 1].yAxis) {
        ...
        yCnt.forEach(function (j) {
          if (
            (j == yObjArr[i].xAxis && xAxisName == "time") ||
    ...
    const findStandalone = graph.links.find(
      (el: any) => el.source == yObjArr[i].id || el.target == yObjArr[i].id
    );
    if (findStandalone == undefined) {
      ...
      if (xAxisName == "time") {
        ...
        x = 10;
        const minHex =
          Number(xAxisArr[0].trim().substring(0, 1).charCodeAt(0)) - 97;
        const maxHex =
          Number(
            xAxisArr[xAxisArr.length - 1].trim().substring(0, 1).charCodeAt(0)
          ) - 97;
  ...
  d3.forceSimulation(graph.nodes)
    ...
    .on("end", function () {
      (
        document.getElementById("dropdown_y") as HTMLElement
      ).style.pointerEvents = "auto";
      (document.getElementById("dropdown_y") as HTMLElement).style.opacity =
        "100%";
      (
        document.getElementById("dropdown_x") as HTMLElement
      ).style.pointerEvents = "auto";
      (document.getElementById("dropdown_x") as HTMLElement).style.opacity =
        "100%";
    });
  ...
  svg.call(
    d3
      ...
      .on("zoom", function (event) {
        ...
        (document.getElementById("time_line") as HTMLElement).style.width =
  ...
  link = container
    ...
    .attr("stroke", function (link: any) {
      const multiEdge = graph.links.filter(
        (el: any) =>
          el.source.id == link.source.id && el.target.id == link.target.id
      );

      if (
        multiEdge.length > 1 &&
        multiEdge.filter((el: any) => el.label == "LINEAGE").length > 0
      ) {
    ...
    .attr("opacity", function (link: any) {
  ...
  node = container
    ...
    .attr("opacity", function (node: any) {
      if (node.label == "DATASET") {
        return 1;
      } else {
        return 0;
    ...
    .on("contextmenu", function (node: any, event: any) {
      event.preventDefault();
      openPopupData(node);
    })
    .on("mouseover", function (event: MouseEvent, node: any) {
   ...
   */
  function updateLink(link: any): void {
    link
      .attr("x1", function (link: any) {
        let x = 10;
        const yFind = yObjArr.find((el: any) => el.id == link.source.id);

        if (
          sourceArr.indexOf(link.source.id) === -1 &&
          link.source.label == "DATASET"
        ) {
          if (xAxisName == "time") {
            x = setSourceObjArrTime(
              yFind,
              link.source.id,
              link.source.createddatetime.substr(0, 10)
            );
          } else {
          ...
          const sourceFind = sourceObjArr.find(
            (el: any) => el.id == link.source.id
          );
          x = sourceFind.x;
        }
      ...
      .attr("x2", function (link: any) {
        let x = 10;
        const yFind = yObjArr.find((el: any) => el.id == link.target.id);

        if (
          sourceArr.indexOf(link.target.id) === -1 &&
          ...
          if (xAxisName == "time") {
            x = setSourceObjArrTime(
              yFind,
              link.target.id,
              link.target.createddatetime.substr(0, 10)
            );
          } else {
 ...
 * @memberof d3canvas
 */
function createGraphLineage(): void {
  ...
  node = container
    ...
    .on("contextmenu", function (event: MouseEvent, node: any) {
      event.preventDefault();
      openPopupData(node);
    })
    .on("mouseover", function (event: MouseEvent, node: any) {
 ...
 */
function updateNode(node: any): void {
  node.attr("transform", function (node: any) {
    let x = fixna(node.x);
    let y = fixna(node.y);

    const sourceFind = sourceObjArr.find((el: any) => el.id == node.id);
    if (sourceFind != undefined) {
      x = sourceFind.x;
      y = sourceFind.y;
    }
 ...
 */
function setEdgeColor(edge: any): string {
 ...
 */
function getRelatedNode2hops(selectedNode: any): void {
  const thisNode = selectedNode.id;
  const target: string[] = [];

  let connected = graph.links.filter(function (link: any) {
  ...
  for (let i = 0; i < target.length; i++) {
    temp = graph.links.filter(function (link: any) {
  ...
  if (checkClick == thisNode) {
    link.attr("opacity", function (link: any) {
      if (
        graph.graph == "LINEAGE" ||
        graph.graph == "BYID" ||
        graph.graph == "data"
    ...
    node.attr("opacity", function (node: any) {
      if (
        graph.graph == "LINEAGE" ||
        graph.graph == "BYID" ||
        graph.graph == "data"
    ...
    node.on("click", function (node: any) {
      if (
        graph.graph == "LINEAGE" ||
        graph.graph == "BYID" ||
        graph.graph == "data"
    ...
    node.attr("opacity", function (node: any) {
      ...
      if (
        connected.map((node: any) => node.source.id).indexOf(node.id) > -1 ||
        connected.map((node: any) => node.target.id).indexOf(node.id) > -1
      ) {
        retVal = 0.5;
      }
 ...
 */
function createDivTime(minDate: string, maxDate: string): void {
  xLabelDist = 31;

  diffXWidth = monthDiff(getDateObject(minDate), getDateObject(maxDate)) + 2;

  const parent = document.getElementById("time_line") as HTMLElement;
  parent.innerHTML = "";
  ...
  for (let i = 0; i < diffXWidth; i++) {
    ...
    div.appendChild(
      document.createTextNode(
        `${monthArr[getMonthCnt % 12]} ${
          parseInt(minDate.substring(6, 10)) + Math.floor(getMonthCnt / 12)
        }`
      )
    );
 ...
 */
function createDivX(minX: string, maxX: string): void {
  xLabelDist = 11;
 ...
 * @returns {number} X value of edge source node
 */
function setSourceObjArrX(find: any, id: string, min: string): number {
  let x = 10;
 ...
 */
async function openPopupData(node: any): Promise<void> {
  dataId = node.id;
  propertiesFileName = node.properties.name;

  const jsonStr = JSON.stringify(node.properties, null, "\t");
  (document.getElementById("clicked_id_properties") as HTMLElement).innerText =
    jsonStr;

  // Get lineage to see, if lineage button should be shown or not
  try {
    const fetchResult = await fetch(
      `/graph/getLineageNodes?dataId=${dataId}`
    ...
    const lineageButton = document.getElementById(
      "lineage_button"
    ) as HTMLElement;
  ...
  if (node.label == "DATA") {
    (document.getElementById("download_data") as HTMLElement).style.display =
      "inline-block";

    // Set title
    (document.getElementById("node_title") as HTMLElement).innerText =
      "File Contents";

    if (node.properties.type == "image") {
      const parent = document.getElementById("file_contents") as HTMLElement;
      ...
      (
        document.getElementById("popup_container") as HTMLElement
      ).style.display = "block";
    } else {
        ...
        .then((data) => {
          ...
          if (data.search('"status":200,') >= 0) {
            ...
            if (dataJSON.message !== "File is too big to be displayed.") {
              (
                document.getElementById("download_data") as HTMLElement
              ).style.display = "none";
            }
    ...
    fetch(
      `/graph/getContainedNodes?datasetId=${node.id}&userId=${D3canvas.userInfo.id}&userRole=${D3canvas.userInfo.role}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
 ...
 */
async function getLineage(graphData: any): Promise<void> {
  closePopupData();
  ...
  // Hide dropdowns and labels
  (document.getElementById("dropdown_x") as HTMLElement).style.display = "none";
  (document.getElementById("dropdown_y") as HTMLElement).style.display = "none";
  (document.querySelector(".x_label_container") as HTMLElement).style.display =
    "none";
  (document.querySelector(".y_label_container") as HTMLElement).style.display =
    "none";
 ...
 * @memberof d3canvas
 */
function closePopupData(): void {
  (document.getElementById("file_contents") as HTMLElement).innerHTML = "";
  (document.getElementById("clicked_id_properties") as HTMLElement).innerText =
    "";
  (document.getElementById("popup_container") as HTMLElement).style.display =
    "none";
 ...
 */
function dayDiff(minDate: Date, maxDate: Date): number {
  return Math.round(
 ...
 */
function monthDiff(minDate: Date, maxDate: Date): number {
  const months =
 ...
 * @memberof d3canvas
 */
function downloadData(downloadButton: HTMLAnchorElement): void {
  downloadButton.href =
    "/data/getDataContent?" +
    new URLSearchParams({
      dataId: dataId,
 ...
 */
function changexAxis(xLabel: string): void {
  storeXAxis = xLabel;
 ...
 */
function changeyAxis(yLabel: string): void {
  storeYAxis = yLabel;
  setGraph(storeXAxis, storeYAxis);
}
 ...
 */
function fillDropdowns(nodes: any[]): void {
  ...
  // Fill drop downs
  const xAxisDropdown = document.getElementById("ul_x") as HTMLElement;
  xAxisDropdown.innerHTML = "";
  const yAxisDropdown = document.getElementById("ul_y") as HTMLElement;
  ...
  for (const property of allProperties) {
    ...
    xDropdownElement.onclick = function (event) {
      ...
      (document.getElementById("dropdown_x") as HTMLElement).blur();

      // Change selected option
      (document.getElementById("get_x_label") as HTMLElement).innerText =
        capitalizeString(property);

      // Change scale legend
      (document.getElementById("x_label") as HTMLElement).innerText =
        capitalizeString(property);
    ...
    yDropdownElement.onclick = function (event) {
      // Prevent bubbling and close the dropdown
      event.stopPropagation();
      (document.getElementById("dropdown_y") as HTMLElement).blur();

      // Change selected option
      (document.getElementById("get_y_label") as HTMLElement).innerText =
        capitalizeString(property);

      // Change scale legend
      (document.getElementById("y_label") as HTMLElement).innerText =
        capitalizeString(property);
 ...
 */
function nodeCompliesToSearchConditions(node: any): boolean {
  for (const searchCondition in displayedSearchConditions) {
